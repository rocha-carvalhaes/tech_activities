...
// Cena do plataformer da Fábrica de Fases. Diferenças em relação ao Slime
// Aventureiro: um CRONÔMETRO que conta do Play até a estrela, e `onWin` recebe o
// tempo final (ms). A cena NÃO desenha overlay de vitória — quem chama (PlayLevel
// ou CreateLevel) decide o que mostrar (ranking de tempo ou botão de salvar).
function GameScene({ attrs, running, onWin }) {
  ...
  const loop = useCallback(() => {
    ...
    draw();
    // Atualiza o cronômetro imperativamente (sem re-render a 60fps).
  ...
  // Entrar em "jogando": reinicia o mundo, zera o cronômetro e dispara o loop.
  useEffect(() => {
    if (!running) return;
    const eng = engineRef.current;
    eng.reset(attrs);
  ...
  return (
    <div className="relative w-full max-w-[480px]">
      ...
      >
        ...
        <rect x="0" y="0" width={VIEW_W} height={VIEW_H} fill="url(#ff-sky)" />
      ...
      {/* Cronômetro (texto atualizado imperativamente em loop) */}
      {running && (
        <div className="absolute top-2 right-3 font-mono text-sm font-semibold text-[#333] bg-white/80 rounded-lg px-2 py-0.5 tabular-nums">
          ⏱ <span ref={timerRef}>0.00s</span>
        </div>
      )}
